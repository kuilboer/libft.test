...
/*   Updated: 2025/05/04 21:18:02 by okuilboe      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int run_memcpy_test(const void *src_data, size_t len, size_t n)
{
	...
	void	*ret_ft = memcpy(dst_ft, src, n);
...
{
	// int		i = 0;
	// unsigned char	src[] = {0XAA, 0XFF, 0X00, 0xDE, 0xAD, 0xBE, 0xEF, '\0'};
	// size_t			n[] = {0, 1, 10, 60};
	// unsigned char	sys_d[50];
	// unsigned char	ft_d[50];
	...
	{
		if (test_ft_memcpy_case(src_data, sizeof(src_data), test_sizes[i]) != 0)
			fails++;
	}
